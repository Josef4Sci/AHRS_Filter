function [Accelerometer,Magnetometer,Gyroscope,qViconReference,time] = createSensDataSimul2()
clear;
...
%%precount
acc_null=[0 0 1];
q_temp=[cosd(wahba_angle/2) 0 sind(wahba_angle/2) 0];
...
for i=1: leng
    if(phase==1&&time(i)>15&&time(i)<20&&abs(generated_acc(i-1,1))<0.01)
        phase=2;
    end
    ...
    if(phase==1)        
        dir=[0.1 0.2 0.05];
        dir=dir/norm(dir);
        q_temp=[cosd(angle/2) dir(1)*sind(angle/2) dir(2)*sind(angle/2) dir(3)*sind(angle/2)];
...
intM=9000:11000;
generated_mag(intM,1)=generated_mag(intM,1)+mag_intensity*0.5*sin(linspace(0,6*pi,2001))';
