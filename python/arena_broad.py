...
test_datasets['03_undisturbed_slow_rotation_C.mat'] = dl.load_broad_dataset(file_name='03_undisturbed_slow_rotation_C.mat')
...
test_filters = {
    ...
    'JustaAHRSv4': {'filter': JustaAHRSv4(w_acc=0.0004, w_mag=0.00014), 'errors': [], 'type': 0},
...
for dataset_name, dat in test_datasets.items():
    ...
    for filter_name, j_filter in test_filters.items():       
        if j_filter['type'] == 1:  # vqf
            ...
            if single_dataset:
                j_filter['errors'].append(error_9D_vqf)
            else:
